...
// CREATE API LINK FROM ALBUM ARTWORK
function getUrlFromArchive(artwork) {
...
function getRandomAlbum() {
  ...
  fetch(
    `${baseDataUrl}?query=tag:${select.value}&offset=${randomOffset}&limit=1&fmt=json`
  )
    .then((resp) => resp.json())
    .then((results) => {
...
function displaySpotify(album) {
  if (album.id === IDS.dream) {
    spotify.src =
      "https://open.spotify.com/embed/album/5ogYKSRRlVAgMzv09HFeIn?utm_source=generator&theme=0";
  } else if (album.id === IDS.omoiyari) {
    spotify.src =
      "https://open.spotify.com/embed/album/7efrHmcFVWTB733vfDt9ey?utm_source=generator&theme=0";
  } else if (album.id === IDS.misfits) {
    spotify.src =
      "https://open.spotify.com/embed/album/636aBJi9ifGPSPacQ0fYCF?utm_source=generator&theme=0";
  } else if (album.id === IDS.alaska) {
    spotify.src =
      "https://open.spotify.com/embed/album/4QZBTJiAHBr2PsWQy2Wg2a?utm_source=generator&theme=0";
  } else if (album.id === IDS.clubExotica) {
    spotify.src =
      "https://open.spotify.com/embed/album/28tDo7nBg0KJWg2HleLKor?utm_source=generator&theme=0";
  } else if (album.id === IDS.wetTennis) {
    spotify.src =
      "https://open.spotify.com/embed/album/1u54eF07irCSSssyDG67R2?utm_source=generator&theme=0";
  } else {
    spotify.src = "";
  }
}
